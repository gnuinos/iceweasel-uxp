...
//   are mapped like so:
//
//   HKCU\SOFTWARE\Classes\.<ext>\      (default)         REG_SZ     IceweaselHTML
//
//   as aliases to the class:
//
//   HKCU\SOFTWARE\Classes\IceweaselHTML\
...
//
//   HKCU\SOFTWARE\Classes\IceweaselURL\  (default)         REG_SZ     <appname> URL
...
//   -------------------------------------------------
//   The following keys are set to make Iceweasel appear in the Start Menu as the
//   browser:
//   
//   HKCU\SOFTWARE\Clients\StartMenuInternet\Iceweasel.EXE\
...
// Iceweasel is the default browser for file handlers since other applications
...
// handler to another application and still have Iceweasel check if it is the
...
static SETTING gSettings[] = {
  ...
  { MAKE_KEY_NAME1("IceweaselHTML", SOC), VAL_OPEN, OLD_VAL_OPEN },

  // Protocol Handler Class - for Vista and above
  { MAKE_KEY_NAME1("IceweaselURL", SOC), VAL_OPEN, OLD_VAL_OPEN },
...
// since they are only checked when Iceweasel is the default browser and if they
// are incorrect they are fixed without notifying the user.
static SETTING gDDESettings[] = {
  // File Handler Class
  { MAKE_KEY_NAME1("Software\\Classes\\IceweaselHTML", SOD) },

  // Protocol Handler Class - for Vista and above
  { MAKE_KEY_NAME1("Software\\Classes\\IceweaselURL", SOD) },
             ...
             LPCWSTR aClassName)
{
  ...
  LPCWSTR progID = isProtocol ? L"IceweaselURL" : L"IceweaselHTML";
 ...
 * This only checks for IceweaselURL and if aCheckAllTypes is set, then
 * it also checks for IceweaselHTML.  Note that those ProgIDs are shared
 * by all Iceweasel browsers.
*/
bool
                                        ...
                                        bool* aIsDefaultBrowser)
{
  ...
  *aIsDefaultBrowser = true;
  ...
  // that was used to launch Iceweasel which is not necessarily a long path.
  ...
  for (; settings < end; ++settings) {
    ...
    if (REG_FAILED(res) ||
      ...
      if (REG_FAILED(res)) {
        ...
        // application when setting Iceweasel as the default browser.
        *aIsDefaultBrowser = false;
        return NS_OK;
      }
  ...
  // Only check if Iceweasel is the default browser on Vista and above if the
  // previous checks show that Iceweasel is the default browser.
  if (*aIsDefaultBrowser) {
  ...
  // account didn't perform a Iceweasel update this will check if Iceweasel is the
  // default browser and if dde is disabled for each handler
  // and if it isn't disable it. When Iceweasel is not the default browser the
  ...
  if (*aIsDefaultBrowser && aForAllTypes) {
    ...
    // update it using the helper application when setting Iceweasel as the
    // default browser.
    if (REG_FAILED(res)) {
      *aIsDefaultBrowser = false;
    }
                                            ...
                                            int32_t aPosition)
{
  ...
  // eventually, the path is "%APPDATA%\Mozilla\Iceweasel\Desktop Background.bmp"
