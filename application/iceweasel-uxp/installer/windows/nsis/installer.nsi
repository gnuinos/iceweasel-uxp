...
Section "-InstallStartCleanup"
  ...
  ${CleanUpdateDirectories} "Mozilla\Iceweasel" "Mozilla\updates"
...
Section "-Application" APP_IDX
  ...
  ; On install always add the IceweaselHTML and IceweaselURL keys.
  ; An empty string is used for the 5th param because IceweaselHTML is not a
  ; protocol handler.
  ${GetLongPath} "$INSTDIR\${FileMainEXE}" $8
  StrCpy $2 "$\"$8$\" -osint -url $\"%1$\""

  ; In Win8, the delegate execute handler picks up the value in IceweaselURL and
  ; IceweaselHTML to launch the desktop browser when it needs to.
...
Section "-InstallEndCleanup"
  ...
  ${Unless} ${Silent}
    ...
    ${If} "$0" == "1"
      ...
      ${If} "$0" != ""
        ClearErrors
        WriteRegStr HKCU "Software\Mozilla\Iceweasel" "OldDefaultBrowserCommand" "$0"
      ${EndIf}
    ...
    ${ElseIfNot} ${Errors}
      ...
      ClearErrors
      WriteRegStr HKCU "Software\Mozilla\Iceweasel" "DefaultBrowserOptOut" "True"
      ${If} ${Errors}
...
Function preSummary
  ...
  ${Unless} ${Errors}
    DeleteRegValue HKLM "Software\Mozilla" "${BrandShortName}InstallerTest"
    ; Check if Iceweasel is the http handler for this user.
    SetShellVarContext current ; Set SHCTX to the current user
    ${IsHandlerForInstallDir} "http" $R9
    ${If} $TmpVal == "HKLM"
      SetShellVarContext all ; Set SHCTX to all users
    ${EndIf}
    ; If Iceweasel isn't the http handler for this user show the option to set
    ; Iceweasel as the default browser.
    ${If} "$R9" != "true"
